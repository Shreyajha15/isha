...
# Comparing the difference between clusters from normalized data vs non-normalized:

# Dunn's Index is improved

# Density of the clusters is relatively distributed and dispersion is relatively less

# The main reason for this difference is due to the fact that all the different types of measurements are brought to one scale when the data is normalized. This removed the effect of large numbers while distance calculation in normalized data.
...
ggplot(df_num, mapping = aes(x = kClusters$cluster)) +
  geom_histogram() +
  labs(title="No of Records in each cluster", x="Cluster")
...
#dunn's index: 0.09507319


# Cluster-1, 10 are the largest
# Cluster-6 is the smallest
# The dispersion/ distance in the data made the least dense cluster diverse
...
# dunn's index: 0.1196583

# Cluster-3 is the largest
# Cluster-4 is the smallest

# Comparing the clusters between non-normalized data and normalized data, we can see that the density of clusters is relatively distributed among more clusters in non-normalized data instead of concentrating in just few clusters in normalized data.

# Dunn's Index is high incase of normalized data compared to non-normalized data
...
data = read.csv('CookieMonsterStacked.CSV')

browseList <- split(x = data$Website, f = data$ID)
...
browseList <- lapply(browseList, unique)
browseList <- as(browseList , "transactions")
browseList[1:2]
```

```{r}

itemFrequency(browseList)

itemFrequencyPlot(browseList)

browsingrules <- apriori(browseList, parameter = list(support = 0.025, confidence = 0.8))

browsingrules_conf <- sort (browsingrules, by="confidence", decreasing=TRUE)

browsingrules_conf <- sort (browsingrules, by="lift", decreasing=TRUE)

inspect(browsingrules_conf)

# Looking at the relative item frequency graph, we can know that average frequency of items is around 0.05

# Using support 0.04 with apriori is not yielding any recommendations (just 2 rows)

# Anything between 0.025 to 0.028 is yeielding results.

# Confidence of 0.8 or more can be found among all the associations in the data. 

# Hence support of 0.025 and confidence of 0.8 is recommended to generate meaningful associations
